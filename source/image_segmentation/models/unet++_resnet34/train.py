...
# =========================================================================
# LƯU Ý: Mở comment phần này nếu bạn cần import modules nội bộ của project
...
# from models.model import UNetPlusPlusResNet34
# =========================================================================
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train U-Net với ResNet-34 backbone.")
    parser.add_argument(
        "--config",
        default="/kaggle/input/datasets/manhtrnnguyn/unet34/config.yaml",
    )
    ...
    output_dir = Path("/kaggle/working/result")
    ...
    # --- KHỞI TẠO DATASETS ---
    base_data_path = "/kaggle/input/datasets/manhtrnnguyn/102oxford/OxfordFlowers102"
    ...
    print(f"Dataset val: {len(val_dataset)} samples")
    ...
    # --- KHỞI TẠO MODEL & THUẬT TOÁN ---
    model = UNetPlusPlusResNet34(num_classes=1).to(device)
    ...
    history, start_epoch, best_dice, epochs_without_improvement = resume_if_available(
        output_dir, model, optimizer, scheduler, device
    )
    ...
    if start_epoch <= int(config["epochs"]):
        ...
        for epoch in range(start_epoch, int(config["epochs"]) + 1):
            ...
            train_metrics = run_epoch(
                ...
                epoch=epoch, phase="train", log_interval=int(config.get("log_interval", 10))
            )
            
            # Val Phase
            val_metrics = run_epoch(
                model, val_loader, criterion, device, 
                epoch=epoch, phase="val", log_interval=int(config.get("log_interval", 10))
            )
            ...
            # Lưu checkpoint trạng thái đầy đủ
            save_checkpoint(
                output_dir / "last_model.pth", model, optimizer, epoch, val_metrics, config, scheduler
            )
            ...
            if val_metrics["dice"] > best_dice:
                ...
                epochs_without_improvement = 0
                save_checkpoint(
                    output_dir / "best_model.pth", model, optimizer, epoch, val_metrics, config, scheduler
                )
                print(f"--> Saved new BEST model with Val Dice: {best_dice:.4f}")
            else:
                epochs_without_improvement += 1
            ...
            # Early Stopping
            if epochs_without_improvement >= int(config.get("early_stopping_patience", 10)):
                print(f"\nEarly stopping triggered sau {epochs_without_improvement} epochs không cải thiện.")
                break
    else:
        print(f"Training already reached epoch {start_epoch - 1}.")

    # =========================================================================
    # POST-TRAINING: LƯU WEIGHTS & ĐÁNH GIÁ TRÊN TẬP TEST
    # =========================================================================
    print("\n" + "="*40)
    print("TRAINING FINISHED. STARTING EVALUATION...")
    print("="*40)

    # 1. Lưu riêng Model Weights của trạng thái cuối (không chứa optimizer để nhẹ file)
    final_weights_path = output_dir / "final_weights_after_train.pth"
    torch.save(model.state_dict(), final_weights_path)
    print(f"[1] Đã lưu model weights (tại epoch cuối) vào: {final_weights_path}")

    # 2. Đánh giá trên tập test bằng BEST MODEL
    best_checkpoint_path = output_dir / "best_model.pth"
    if best_checkpoint_path.exists():
        print(f"\n[2] Loading BEST model từ {best_checkpoint_path} để thực hiện Test...")
        checkpoint = torch.load(best_checkpoint_path, map_location=device)
        model.load_state_dict(checkpoint["model_state_dict"])
    else:
        print("\n[!] Không tìm thấy best_model.pth, sẽ test bằng weights ở epoch cuối cùng.")

    print("Đang chạy trên tập Test...")
    test_metrics = run_epoch(
        model, test_loader, criterion, device, 
        epoch=None, phase="test", log_interval=int(config.get("log_interval", 10))
    )
    ...
    print(f"Đã lưu kết quả test vào: {output_dir / 'test_metrics.json'}")
