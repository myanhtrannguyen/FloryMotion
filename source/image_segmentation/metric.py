...
def _binarize(x: ArrayLike, threshold: float = 0.5) -> np.ndarray:
    ...
    # If x is already boolean, keep it.
    if x.dtype == np.bool_:
...
def precision_score(
    ...
    for p, g in zip(pred, target):
        ...
        if tp + fp == 0:
            ...
            scores.append((tp + eps) / (tp + fp + eps))
...
def _surface_distances(pred_mask: np.ndarray, gt_mask: np.ndarray) -> np.ndarray:
    """
    Compute distances from the surface of pred_mask to the surface of gt_mask
    and from the surface of gt_mask to the surface of pred_mask.
    """
    ...
    pred_border = np.logical_xor(
        pred_mask, ndimage.binary_erosion(pred_mask, structure=structure)
    )
    gt_border = np.logical_xor(
        gt_mask, ndimage.binary_erosion(gt_mask, structure=structure)
    )

    # Distance transform to nearest boundary pixel.
    ...
    return np.concatenate([distances_pred_to_gt, distances_gt_to_pred]).astype(np.float32)
...
def hd95(
    ...
    Unit: pixels, unless masks are resampled with physical spacing elsewhere.
    """
    pred = _binarize(pred, threshold)
    target = _binarize(target, threshold)
    _validate_shapes(pred, target)
    ...
    for p, g in zip(pred, target):
        ...
        if np.isinf(distances).any():
            # If one mask is empty and the other is not, return image diagonal.
            h, w = p.shape
            scores.append(float(np.sqrt(h * h + w * w)))
        else:
...
def boundary_f_score(
    ...
    pixels of a ground-truth boundary pixel.
    ...
    for p, g in zip(pred, target):
        ...
        p_boundary_dilated = ndimage.binary_dilation(
            p_boundary, structure=structure, iterations=tolerance
        )
        g_boundary_dilated = ndimage.binary_dilation(
            g_boundary, structure=structure, iterations=tolerance
        )

        precision_match = np.logical_and(p_boundary, g_boundary_dilated).sum()
        recall_match = np.logical_and(g_boundary, p_boundary_dilated).sum()

        boundary_precision = (precision_match + eps) / (p_boundary.sum() + eps)
        boundary_recall = (recall_match + eps) / (g_boundary.sum() + eps)
...
def compute_all_metrics(
    ...
    Returns:
        dict with keys:
        dice, iou, precision, hd95, boundary_f_score,
    ...
    return {
        ...
        "precision": precision_score(pred, target, threshold=threshold),
        "hd95": hd95(pred, target, threshold=threshold),
        "boundary_f_score": bf,
